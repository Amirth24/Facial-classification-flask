...
def dashboard():
    ...
    if not session.get('userid') or not os.path.exists(os.path.join(current_app.config['UPLOAD_FOLDER'],str(session['userid']))) or os.listdir(os.path.join(current_app.config['UPLOAD_FOLDER'],str(session['userid']))) == []:
